...
public class JBIG2PageTest {
	...
	public void composeDisplayTest() throws IOException, JBIG2Exception {
		...
		BufferedImage b = doc.getPage(pageNumber).getBitmap()
				.getBufferedImage(
						new JBIG2ReadParam(1, 1, 0, 0, new Rectangle(166, 333, 555, 444),
								null));
	...
	public void composeTestWithDurationCalc() throws IOException, JBIG2Exception {
		...
		for (int i = 0; i < runs; i++) {
			...
			JBIG2Document doc = new JBIG2Document(iis, false);
			JBIG2ReadParam param = new JBIG2ReadParam(1, 1, 0, 0, new Rectangle(0, 0,
					doc.getPage(pageNumber).getWidth(), doc.getPage(pageNumber)
							.getHeight()), new Dimension(doc.getPage(pageNumber).getWidth(),
					doc.getPage(pageNumber).getHeight()));
	...
	public void composeTestWithDurationCalcAggregate() throws IOException,
		...
		for (int j = 1; j <= pages; j++) {
			...
			for (int i = 0; i < runs; i++) {
				long time = System.currentTimeMillis();
				JBIG2Document doc = new JBIG2Document(iis, false);
				doc.getPage(j).getBitmap().getBufferedImage(
						new JBIG2ReadParam(1, 1, 0, 0, null, null));
				long duration = System.currentTimeMillis() - time;
